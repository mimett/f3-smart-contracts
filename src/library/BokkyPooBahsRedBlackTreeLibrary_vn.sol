...
library BokkyPooBahsRedBlackTreeLibrary {
	...
	function getNode(
		Tree storage self,
		uint key
	) internal view returns (uint _returnKey, uint _parent, uint _left, uint _right, bool _red) {
		require(exists(self, key));
		return (key, self.nodes[key].parent, self.nodes[key].left, self.nodes[key].right, self.nodes[key].red);
	}

	function insert(Tree storage self, uint key, function(uint256, uint256, uint256) returns (bool) lessThan, uint256 data) internal {
		require(key != EMPTY);
		require(!exists(self, key));
		uint cursor = EMPTY;
